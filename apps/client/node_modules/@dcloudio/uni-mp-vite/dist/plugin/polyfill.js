...
function rewriteCompileScript() {
    ...
    compiler.compileStyleAsync = (options) => {
        if (process.env.UNI_APP_STYLE_ISOLATION_VERSION === '2' &&
            ...
            if ((0, uni_cli_shared_1.isAppVue)(options.filename)) {
                options.source = `@import "/uvue.wxss";\n` + options.source;
            }
            else {
                const { styleIsolation, isPage } = (0, uni_cli_shared_1.findMiniProgramComponentStyleIsolation)(options.filename) || {};
                if (!isPage) {
    ...
    compiler.compileScript = (sfc, options) => {
        ...
        if (process.env.UNI_APP_STYLE_ISOLATION_VERSION === '2' &&
            ...
            // @ts-expect-error
            options.__isPage = (0, uni_cli_shared_1.isMiniProgramPageFile)(sfc.filename, process.env.UNI_INPUT_DIR);
            const styleIsolation = (0, uni_cli_shared_1.parseStyleIsolation)(res);
            if (styleIsolation) {
                (0, uni_cli_shared_1.updateMiniProgramComponentStyleIsolation)(sfc.filename, styleIsolation, 
                // @ts-expect-error
                options.__isPage);
            }
        }
...
function rewriteCompilerSfcParse() {
    ...
    compilerSfc.parse = (source, options) => {
        ...
        if (process.env.UNI_APP_STYLE_ISOLATION_VERSION === '2' &&
            ...
            res.descriptor.styles = [(0, uni_cli_shared_1.createDefaultSFCStyleBlock)(source)];
        }
        return res;
    };
}
